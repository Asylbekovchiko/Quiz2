...
public class level1 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.unilvlv);

        fon= MediaPlayer.create(this, R.raw.v);
        correct=MediaPlayer.create(this, R.raw.correct);
        incorrect=MediaPlayer.create(this, R.raw.incorrect);

        ImageView backuniversal1lvl = (ImageView) findViewById(R.id.back1);
        backuniversal1lvl.setImageResource(R.drawable.backfirstlvl);


        TextView tb = findViewById(R.id.lvl);
        tb.setTextColor(Color.BLACK);

        TextView tvlvl=findViewById(R.id.lvl);
        tvlvl.setText(R.string.level1);
        ...
        dialog.setContentView(R.layout.firstlvldialog);
...
///////////////////////////////////
        ...
        final Animation a = AnimationUtils.loadAnimation(level1.this, R.anim.alpha);//в
        numLeft=random.nextInt(20);
        imgLeft.setImageResource(array.imagesfirst[numLeft]);

        numRight=random.nextInt(20);
        while(array.value[numLeft]==array.value[numRight]){
            numRight=random.nextInt(20);

        }
        imgRight.setImageResource(array.imagesfirst[numRight]);
        ...
        imgLeft.setOnTouchListener(new View.OnTouchListener() {
            ...
            if(event.getAction()==MotionEvent.ACTION_DOWN){
                ...
                if(array.value[numLeft]>array.value[numRight]){
                ...
                if(count==20){
                    ...
                    imgLeft.setImageResource(array.imagesfirst[numLeft]);//достаем из массива картинку(например рандомное число 1, следовательно достается картинка под номером один(отсчет идет с 0 до 9))
                    imgLeft.startAnimation(a);
                    numRight=random.nextInt(20);//рандомное число для right
                    while(array.value[numLeft]==array.value[numRight]){
                        numRight=random.nextInt(20);
                    }
                    imgRight.setImageResource(array.imagesfirst[numRight]);//достаем из массива картинку(например рандомное число 1, следовательно достается картинка под номером один(отсчет идет с 0 до 9))
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if(event.getAction()==MotionEvent.ACTION_DOWN){
                    ...
                    if(count==20){
                        ...
                        numLeft=random.nextInt(19);     //рандомное число для imgleft and textleft
                        imgLeft.setImageResource(array.imagesfirst[numLeft]);//достаем из массива картинку(например рандомное число 1, следовательно достается картинка под номером один(отсчет идет с 0 до 9))
                        imgLeft.startAnimation(a);
